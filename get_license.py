...
#Python code to download certain files from a secure server. The server can be accessed via a VPN connection.
...
#Once a connection to the server has been verified and the network share has been mounted, files can be pulled from the server to the local computer's desktop.
#It saves, and sorts, each downloaded filenumber in a list for reference.
...
def mount_network_share(alt_mount,os_type):
        ...
        print()
        want_to_mount = input('Do you want to try to connect to the server now? (Y/N): ').lower()
        print()
        ...
        if want_to_mount == 'y':
            if not os.path.exists(alt_mount):
                os.makedirs(alt_mount,0o755) #Leading zeros not allowed so 0755 permissions must be set as 0o755
                
            #Get login username for the remote server.
            username = str(input('Please type your login username at ' + server + ': '))

            #Find out from what OS type you are trying to connect to the remote server and mounts the network share.
